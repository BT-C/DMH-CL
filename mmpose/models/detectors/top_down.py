...
class TopDown(BasePose):
    ...
    def forward_test(self, img, img_metas, return_heatmap=False, **kwargs):
        ...
        # import os
        # import cv2
        # target_dir = '/home/chenbeitao/data/code/Test/mask-code'
        # for img_id, img_meta in enumerate(img_metas):
        #     if img_meta['image_file'].split('/')[-1] == '000000089078.jpg':
        #         print('-' * 30)
                
        #         origin_img = img[img_id].cpu().numpy()
        #         origin_img = (origin_img * 255).transpose(1, 2, 0)
        #         heatmaps = output_heatmap[img_id]
        #         for heatmap_id, small_heatmap in enumerate(heatmaps):
        #             heatmap = cv2.resize(small_heatmap, (221, 573))
        #             heatmap = (heatmap - heatmap.min()) / (heatmap.max() - heatmap.min())
        #             heatmap = np.uint8(255 * heatmap)
        #             heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        #             # heatmap_img = heatmap * 0.5 + origin_img
        #             heatmap_img = heatmap
        #             heatmap_img = np.clip(heatmap_img, 0, 255)
        #             heatmap_img = heatmap_img.astype(np.uint8)
        #             cv2.imwrite(os.path.join(target_dir, f'heatmap_{heatmap_id}.jpg'), heatmap_img)
        #         assert 0 == 1
        # ==================================================================
